...
def save_video_info(video_info_path, video_data, set_header = False):
    """
    Store video info (video_data) into a csv file specified by video_info_path
    if set_header is True
    """
    with open(video_info_path, 'a', newline="") as f:
        writer = csv.writer(f)
